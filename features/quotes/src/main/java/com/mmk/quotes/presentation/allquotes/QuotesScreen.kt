...
@Composable
fun QuotesScreen(viewModel: QuotesViewModel = koinViewModel()) {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                when (uiState) {
                    UiState.HasData -> QuotesDataList(
                        quotes = quotes, modifier = Modifier.fillMaxSize().padding(top = 16.dp)
                    )
        ...
        items(quotes) {
            it?.let {
                ...
                QuoteItem(
                    ...
                    onClickLikeButton = {
                        Toast.makeText(context, "Clicked Like button", Toast.LENGTH_SHORT).show()
                    },
                    onClickShareButton = {
        ...
        item {
            Box(
                ...
                if (quotes.loadState.append is LoadState.Loading) {
                    MyCircularProgressBar(modifier = Modifier.padding(top = 8.dp, bottom = 50.dp))
                }
...
private fun QuotesScreenPreview() {
    MyApplicationTheme {
        QuotesScreen(uiState = UiState.HasData, quotes = getTestQuotes().collectAsLazyPagingItems())
    }
}
...
private fun EmptyQuotesPreview() {
    MyApplicationTheme {
        QuotesScreen(uiState = UiState.NoData, quotes = getTestQuotes().collectAsLazyPagingItems())
    }
}
...
private fun LoadingQuotesPreview() {
    MyApplicationTheme {
        QuotesScreen(uiState = UiState.Loading, quotes = getTestQuotes().collectAsLazyPagingItems())
    }
}
